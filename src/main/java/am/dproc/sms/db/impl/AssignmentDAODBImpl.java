...
public class AssignmentDAODBImpl implements AssignmentDAO {
	 ...
	 * "        WHERE ASSIGNMENT_ID = ?\r\n" + "    ),\r\n" + "    ''\r\n" + ")";
	 */
	private static final String GET_ALL_ASSIGNMENTS_BY_LESSON_ID_AND_TEACHER_ID = "SELECT * FROM mydb.ASSIGNMENT WHERE LESSON_ID = ? AND TEACHER_ID = ?";
	...
	private static final String ADD_ASSIGNMENT = "INSERT INTO mydb.ASSIGNMENT (START_DATE, END_DATE, TITLE, DESCRIPTION, TEACHER_ID, LESSON_ID, CREATION_DATE) VALUES (?, ?, ?, ?, ?, ?, ?)";
	...
	public Integer addAssignment(Assignment assignment) {
		return jdbctemplate.update(ADD_ASSIGNMENT, new Object[] { assignment.getStartDate(), assignment.getEndDate(),
				assignment.getTitle(), assignment.getDescription(), assignment.getLessonId(), assignment.getTeacherId(), System.currentTimeMillis() });
	}
